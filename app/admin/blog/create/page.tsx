...
export default async function AdminCreateBlogPage() {
  ...
  async function handleCreate(payload: Record<string, unknown>) {
    ...
    await createBlogPost(payload as Record<string, unknown>);
    redirect("/admin/blog");
  }
        ...
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">Create rich content with SEO, images, and publishing controls.</p>
            </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <AdminBlogForm onSubmit={handleCreate} submitLabel="Create Post" />
          </div>
        </div>
      </div>
    </AdminRealtimeProvider>
